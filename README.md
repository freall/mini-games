...
| 模块 | 练什么 | 怎么玩 |
| --- | --- | --- |
| 📡 发报 `send` | 把码**发出去**（手上功夫） | 面板给一条电文（真词，如 SOS / TEAM），用电键逐字发：短按出点、按住出划、松手停顿即字符结束 |
| 🎧 抄收 `recv` | 把码**听懂/看懂**（耳朵和眼睛） | 电台发来一段码（可切「看码」读点划条），从形近干扰项里选出是哪个字符；速度按 WPM 递进 |
| 🔢 数字 `digit` | 数字的**编解码** | 交替出题：编码题给数字让你发码；解码题听/看码从 0~9 里选。附「数字的律」提示 |

三个模块共用同一套教学机制：字符按 6 组逐关解锁、答错立刻回放正确码并给中文口诀、
错字进「待复习」后面还会再考、每字符熟练度（对 +2 / 错 −1，≥6 算掌握）长期存在本地，
门户显示「已掌握 N/36」。
...
5. **延迟执行的状态重置会吞掉玩家抢先的输入，而"推进"要分清推什么**
   摩尔斯一条电文发完有 450ms 停顿，停顿里的重置会清空输入缓冲 —— 手快的玩家在那一瞬间
   按下的第一个符号会被吃掉。解法：按键时先 flush 掉待执行的动作。
   **但 flush 时不能一律调 `advance()`**：一条电文发完后索引已经指向下一句的第一个字符，
   这时再 advance 就会**跳过这个字符**（加了抄收模块后踩到的，表现为连发三字第二字莫名判错）。
   正解是把待执行动作分成 `begin`（开始新一题）与 `advance`（前进一题）两种，按场景登记。

6. **`position:fixed` 的覆盖层会挡住它下面的所有控件**
   摩尔斯把覆盖层改成铺满视口后，棋盘上的「发报/抄收/数字」标签在开始面板打开时**点不到**了
   （覆盖层盖住了它）。解法：在开始/结算面板里也放一份同样的模块选择器，
   两边都用 `data-mode` 属性 + 一个 document 级委托处理器统一分发与高亮。

7. **`site/` 和 `dist/` 都是产物，不要手改**
